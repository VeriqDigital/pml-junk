...
const leadSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    preferredTime: z.string().trim().regex(/^\d{2}:\d{2}$/),
...
const leadLabels: Record<Lead["type"], string> = {
  ...
  service: "Junk removal quote request",
};
...
const formatLeadForOwner = (lead: Lead) => {
  ...
  if (lead.type === "service") {
    rows.push(
      `Pickup address: ${lead.pickupAddress}`,
      `Item type: ${lead.itemType}`,
      `Preferred pickup: ${lead.preferredDate} at ${lead.preferredTime}`,
    );
  }
...
const getAutoReplyText = (type: Lead["type"]) => {
  if (type === "service") {
    return `Thanks for requesting a junk removal quote with ${siteConfig.name}. We received your pickup details and will follow up to review the job and scheduling.`;
  }
